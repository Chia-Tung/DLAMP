...
def gen_TW_only_terrain():
    ...
    with open("config/data/rwrf.yaml", "r") as stream:
        ...
        lat_range = data_config["image_lat"]
        lon_range = data_config["image_lon"]
        resolution = data_config["image_res"]

    data_gnrt = DataGenerator(lat_range, lon_range, resolution)
    ...
    target_lat = data_gnrt.yield_data(datetime(2022, 10, 1, 0), dc_lat)
    target_lon = data_gnrt.yield_data(datetime(2022, 10, 1, 0), dc_lon)
...
def extract_landmask_from_ncfile():
    with open("config/data/rwrf_dense.yaml", "r") as stream:
        data_config = yaml.safe_load(stream)
        data_shape = data_config["data_shape"]
        img_shape = data_config["image_shape"]

    filename = gen_path(datetime(2022, 10, 1, 0))
